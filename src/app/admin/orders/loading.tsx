    ...
    <div className="space-y-6 max-w-7xl">
      ...
      <div className="bg-white rounded-xl border border-slate-200 overflow-hidden">
        <div className="border-b border-slate-200 bg-slate-50 px-4 py-3 flex gap-8">
          {['Order', 'Customer', 'Restaurant', 'Items', 'Amount', 'Payment', 'Status', 'Date'].map((h) => (
            <div key={h} className="h-3 w-16 rounded bg-slate-200 skeleton-shimmer" />
          ))}
        </div>
        {Array.from({ length: 8 }).map((_, i) => (
          <div key={i} className="px-4 py-4 border-b border-slate-100 flex gap-8">
            {[80, 120, 120, 40, 80, 80, 80, 80].map((w, j) => (
